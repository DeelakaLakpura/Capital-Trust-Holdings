...
class GuestController extends Controller
{
  ...
  public function rent(string $item,Request $request)
  {
    ...
    if ($item == 'apartment') {
      ...
      $properties = Property::where('deal_type', 'forRent')->where('property_type', 'House')->where('status', 'active')
        ->with('mainImage') // Fetch one image per property
       ;
    } elseif ($item == 'residantial-land') {
      $properties = Property::where('deal_type', 'forRent')->where('property_type', 'Residential Land')->where('status', 'active')
        ->with('mainImage') // Fetch one image per property
       ;
      ...
      ->where(function ($query) {
          $query->where('property_type', 'Commercial Land')
  ...
  public function buy(string $item, Request $request)
  {
    ...
    if ($item == 'apartment') {
      ...
      $properties = Property::where('deal_type', 'forBuy')->where('property_type', 'Residential Land')->where('status', 'active')
        ->with('mainImage');
    } elseif ($item == 'hotels-and-villas') {
